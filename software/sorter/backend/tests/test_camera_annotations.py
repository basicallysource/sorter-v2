...
def test_runtime_annotation_provider_emits_role_bound_layers() -> None:
    ...
    # the separate "ghosts" category so the stream can toggle them. Shadow
    # tracker overlay is no longer wired by default — operators only see the
    # production primary tracker on camera streams.
    assert [overlay.category for overlay in overlays] == [
        "detections",
        "ghosts",
    ]
    assert any(isinstance(o, RuntimeTrackOverlay) for o in overlays)
    assert not any(isinstance(o, RuntimeShadowTrackOverlay) for o in overlays)
