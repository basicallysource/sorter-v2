...
<div class="border border-gray-200 bg-white p-3">
	<div class="mb-2 flex items-center justify-between">
		...
		{#if onCancel}
			<button onclick={onCancel} class="p-1 text-gray-400 hover:text-gray-600" aria-label="Close">
	...
	<input type="text" bind:value={query} oninput={handleInput}
		...
		class="mb-2 w-full border border-gray-300 px-3 py-2 text-sm focus:border-[#D01012] focus:outline-none focus:ring-1 focus:ring-[#D01012]" />
	<div class="mb-2 flex items-center gap-2">
		<input type="number" bind:value={minYear} oninput={handleInput}
			placeholder="From year" min="1949" max="2030"
			class="w-24 border border-gray-300 px-2 py-1 text-xs focus:border-[#D01012] focus:outline-none focus:ring-1 focus:ring-[#D01012]" />
	...
	{:else if searched && results.length === 0}
		<div class="py-4 text-center text-xs text-gray-400">No sets found</div>
	{:else if results.length > 0}
		<div class="max-h-64 space-y-1 overflow-y-auto">
			{#each results as set (set.set_num)}
				<button onclick={() => onSelect(set)}
					class="flex w-full items-center gap-3 border border-gray-100 p-2 text-left hover:bg-gray-50">
					{#if set.img_url}
						<img src={set.img_url} alt={set.name} class="h-12 w-12 shrink-0 object-contain" />
					{:else}
						<div class="flex h-12 w-12 shrink-0 items-center justify-center bg-gray-100 text-xs text-gray-400">N/A</div>
					{/if}
					<div class="min-w-0 flex-1">
						<div class="truncate text-sm font-medium text-gray-800">{set.name}</div>
						<div class="text-xs text-gray-400">{set.set_num} · {set.year} · {set.num_parts} parts</div>
					</div>
				</button>
			{/each}
		</div>
