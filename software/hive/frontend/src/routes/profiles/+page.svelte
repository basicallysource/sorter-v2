...
{:else if profiles.length === 0}
	<div class="border border-[#E2E0DB] bg-white p-6 text-sm text-[#7A7770]">You have not created any profiles yet.</div>
{:else}
	<div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
		{#each profiles as profile (profile.id)}
			...
			<a href={profile.is_owner ? `/profiles/${profile.id}/edit` : `/profiles/${profile.id}`}
				class="group flex flex-col border border-[#E2E0DB] bg-white transition-colors hover:border-[#7A7770]">
				<!-- Header -->
				<div class="px-4 pt-4 pb-3">
					<div class="flex items-start justify-between gap-2">
						<div class="min-w-0">
							<h2 class="flex items-center gap-2 truncate text-sm font-semibold {profile.visibility === 'public' ? 'text-[#0055BF]' : 'text-[#1A1A1A]'}">
							...
							{#if profile.description}
								<p class="mt-0.5 truncate text-xs text-[#7A7770]">{profile.description}</p>
							{/if}
						</div>
						...
						<div class="space-y-1.5">
							{#each activeRules.slice(0, 6) as rule}
								<div class="flex items-center gap-2 text-xs">
									...
									{/if}
									<span class="truncate text-[#1A1A1A]">{rule.name}</span>
									{#if rule.rule_type === 'set' && rule.set_num}
										<span class="shrink-0 font-mono text-[10px] text-[#7A7770]">{rule.set_num}</span>
									{:else if rule.condition_count > 0}
										<span class="shrink-0 text-[10px] text-[#7A7770]">{rule.condition_count} cond{rule.condition_count !== 1 ? 's' : ''}</span>
									{/if}
									{#if rule.child_count > 0}
										<span class="shrink-0 text-[10px] text-[#7A7770]">+{rule.child_count} sub</span>
									{/if}
								</div>
							{/each}
							{#if activeRules.length > 6}
								<div class="text-[10px] text-[#7A7770]">+{activeRules.length - 6} more rules</div>
							{/if}
						</div>
					</div>
				{:else if rules.length === 0}
					<div class="border-t border-[#E2E0DB] px-4 py-2.5">
						<span class="text-xs text-[#7A7770]">No rules defined</span>
					</div>
				{/if}
...
{#if deleteTarget}
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onkeydown={(e) => { if (e.key === 'Escape') deleteTarget = null; }} onclick={() => deleteTarget = null}>
		...
		>
			<h3 class="text-lg font-semibold text-[#1A1A1A]">Delete Profile</h3>
			<p class="mt-2 text-sm text-[#7A7770]">
				Are you sure you want to delete <span class="font-medium text-[#1A1A1A]">{deleteTarget.name}</span>? This action cannot be undone.
			</p>
			{#if error}
				<div class="mt-3 bg-[#D01012]/8 p-2 text-sm text-[#D01012]">{error}</div>
			{/if}
