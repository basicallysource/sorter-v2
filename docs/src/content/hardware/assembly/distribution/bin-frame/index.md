...
4. **[Bottom interface]({{ '/hardware/assembly/distribution/bin-frame/bottom-interface/' | relative_url }})** — the Lazy Susan stage. It bolts up under the bottom layer's spokes once that layer is standing, which is why it comes after them rather than before.
5. **[Bin retainers]({{ '/hardware/assembly/distribution/bin-frame/bin-retainers/' | relative_url }})** — the same twelve rails on every bin layer. Steps 2 and 3 both send you here, once per layer.
