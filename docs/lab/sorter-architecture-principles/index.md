...
For the Sorter, that means the runtime path of the active machine topology:

- C-channel feeding: `C1 -> C2 -> C3 -> C4 -> Distributor`
- Belt feeding: `B1 -> C3 -> C4 -> Distributor`

Both share the C3 → C4 → Distributor tail; a topology swaps the upstream
feeding stages, never the shape of the path.
...
| Detection / channel perception | perception service | owns detectors + per-channel `ChannelState`; consumers read state, they do not run inference |
