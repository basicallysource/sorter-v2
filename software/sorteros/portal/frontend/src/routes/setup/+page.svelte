<script lang="ts">
	...
	// Kept in lock-step with the portal backend's DEFAULT_HIVE_URL and the
	// sorter's DEFAULT_HIVE_URL.
	const HIVE_URL = 'https://hive.basically.website';

	type Stage = 'loading' | 'pick' | 'auth' | 'submitting' | 'handoff';
	...
	// Just an id — the keypair lives on the Hive lookup page (https).
	...
	{:else if stage === 'auth' || stage === 'submitting'}
		<form class="flex flex-1 flex-col gap-4" onsubmit={submit}>
			...
			<label class="block">
				...
				/>
				<div class="mt-1 text-xs text-[var(--color-text-muted)]/80">
					This becomes the device name on your network ({hostnameDraft.trim() || status?.hostname || 'sorter'}.local).
				</div>
			</label>
			...
			>
				{showAdvanced ? '− Hide advanced' : '+ Advanced (SSH key)'}
			</button>
