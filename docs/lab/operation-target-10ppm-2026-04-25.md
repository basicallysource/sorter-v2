...
| P0 | Drive the C4 classify + distributor cycle next (the new bottleneck) | With section feeder removed as the cap, C4's `front_already_requested = 2122` vs `accepted = 14` shows the time budget is dominated by C4 transport waiting for classification + distributor handoff. The simulated chute is fast (1 s + 0.4 s settle + 1.5 s fall = ~3 s/piece), but the classify pre-trigger and handoff horizon currently leave the front piece in `front_already_requested` for most of its rotation. | **Step 1 done**: `CarouselC4Handler` built (Main-style sequential scheduler), `c4_mode` flag scaffolded, 576 tests passing. **Step 2 (next session)**: route C4 transport + handoff + eject decisions through the handler when `c4_mode = "carousel"` — needs RuntimeC4 to expose a "decisions disabled" flag so the two paths don't fight for the hardware. Then live A/B against `c4_mode = "runtime"`. |
