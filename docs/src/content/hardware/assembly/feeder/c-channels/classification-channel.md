...
The lamp's arm mount feeds up into one of the dovetails around the outside of the channel wall, from the bottom edge, and clips over the top: the 8th dovetail from the exit, counting anticlockwise seen from above, which is the last one before the channel above covers the wall. There is no screw in that joint. The joint itself happens at step 9 of [arranging C-channels]({{ '/hardware/assembly/feeder/arranging-c-channels/' | relative_url }}), with the channels standing, and the lamp it carries is built on the [classification camera lamp]({{ '/hardware/assembly/feeder/camera-lamp/classification-camera-lamp/' | relative_url }}) page.
