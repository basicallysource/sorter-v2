# SortHive Sorter Client

Python client for the SortHive machine API. Used by sorting machines to send heartbeats and upload classification samples.
...
from sorthive_client import SortHiveClient

client = SortHiveClient(
    api_url="https://sorthive.example.com",
...
```bash
curl -X POST https://sorthive.example.com/api/machine/heartbeat \
...
```bash
curl -X POST https://sorthive.example.com/api/machine/upload \
  -H "Authorization: Bearer YOUR_TOKEN" \
