...
<svelte:head>
	<title>Register - SortHive</title>
</svelte:head>

<div class="flex min-h-[80vh] items-center justify-center">
	<div class="w-full max-w-sm border border-[#E2E0DB] bg-white p-8">
		<h1 class="mb-6 text-center text-2xl font-bold text-[#1A1A1A]">Create an account</h1>

		{#if error}
			<div class="mb-4 bg-[#FEF2F2] p-3 text-sm text-[#9B1D20]">{error}</div>
		{/if}

		<form onsubmit={handleSubmit} class="space-y-4">
			<div>
				<label for="displayName" class="mb-1 block text-sm font-medium text-[#1A1A1A]">Display Name</label>
				<input
					id="displayName"
					type="text"
			...
			<div>
				<label for="email" class="mb-1 block text-sm font-medium text-[#1A1A1A]">Email</label>
				<input
					id="email"
					type="email"
					bind:value={email}
			...
			<div>
				<label for="password" class="mb-1 block text-sm font-medium text-[#1A1A1A]">Password</label>
				<input
					id="password"
					type="password"
					bind:value={password}
		...
		{#if githubEnabled}
			<div class="my-5 flex items-center gap-3">
				<div class="h-px flex-1 bg-[#E2E0DB]"></div>
				<span class="text-xs font-medium uppercase tracking-wide text-[#7A7770]">or</span>
				<div class="h-px flex-1 bg-[#E2E0DB]"></div>
			</div>

			<a
				href={api.githubLoginUrl()}
				class="flex w-full items-center justify-center gap-3 border border-[#E2E0DB] px-4 py-2 text-sm font-medium text-[#1A1A1A] hover:bg-[#F7F6F3]"
			>
		...
		<p class="mt-4 text-center text-sm text-[#7A7770]">
			Already have an account?
			<a href="/login" class="text-[#D01012] hover:underline">Sign in</a>
		</p>
	</div>
</div>
