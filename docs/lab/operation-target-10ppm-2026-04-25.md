...
| P0 | Decide architectural path: keep multi-layer backpressure or revert to Main-style sectional logic | Live shadow observer evidence (2026-04-26 17:08–17:38, ``logs/sector_shadow.jsonl``) shows sorthive's C1 backpressure stack (``vision_density_clump``, ``vision_target_band``, ``c4_backpressure_holding``, ``transport_bad_actor_cluster``) is the dominant cap on PPM. Disabling them progressively brought PPM 0.0 → 2.17, but Main's sectional logic delivered 6-7 PPM at ~1/10 the LOC. | Options on the table: (A) port section-based feeder decisions from ``main`` into ``rt/`` (keep BoxMot for image collection — operator's hard constraint — but interpret tracks as section occupancy); (B) keep architecture and tighten the gates that the shadow observer has flagged as false-positive; (C) hybrid — start by removing ``vision_density_clump`` and the C2→C3 lease while keeping the rest. A is highest payoff but biggest refactor; B is lower-risk but caps below Main; C is the smallest viable step toward A. |
