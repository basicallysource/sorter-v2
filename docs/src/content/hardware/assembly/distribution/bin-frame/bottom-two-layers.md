...
No heat inserts on this assembly. Every printed part here takes a self-tapping {% include fastener.html size="M5" variant="socket-button" length="16" %} screw straight into the plastic, the same as the [external bracket]({{ '/hardware/parts/external-bracket/' | relative_url }}).
