...
<div class="border border-[#E2E0DB] bg-white p-3">
	...
	<div class="mb-2 flex items-center gap-2">
		<input type="number" bind:value={minYear} oninput={handleInput}
			...
			class="w-24 border border-[#E2E0DB] px-2 py-1 text-xs focus:border-[#D01012] focus:outline-none focus:ring-1 focus:ring-[#D01012]" />
	...
	{:else if searched && results.length === 0}
		<div class="py-4 text-center text-xs text-[#7A7770]">No sets found</div>
			...
			{#each results as set (set.set_num)}
				<button onclick={() => onSelect(set)}
					...
					<div class="min-w-0 flex-1">
						...
						<div class="text-xs text-[#7A7770]">{set.set_num} · {set.year} · {set.num_parts} parts</div>
					</div>
				</button>
			{/each}
		</div>
