...
The fan is the same 24 V WINSINN 4010 as the one in the [control board housing]({{ '/hardware/electronics/installation/control-board-housing/' | relative_url }}), on the same XH2.54 2-pin lead. The Orange Pi has no 24 V rail and no fan header of its own, so the lead has to reach a switched 24 V port on the control board, the same LED ports that page uses, with its bypass jumper bridged. Which port that is has not been recorded from a build yet.
