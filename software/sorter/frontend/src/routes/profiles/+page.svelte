<script lang="ts">
	...
	function sourceLabel(target: HiveTargetLibrary): string {
		return target.name || target.url || 'Unknown source';
	}

	function targetWebUrl(target: HiveTargetLibrary): string | null {
	...
	function lastUsedAt(target: HiveTargetLibrary, profile: SortingProfileSummary): string | null {
