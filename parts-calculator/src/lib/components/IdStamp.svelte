...
{#if where === 'global'}
	...
	<Popover width="18rem" placement="bottom-end">
	...
	{#if where === 'viewport'}
		<div class="pointer-events-auto flex flex-wrap items-center gap-x-3 gap-y-1.5 border border-border bg-[var(--color-surface)]/95 px-2.5 py-1.5 text-xs shadow-sm backdrop-blur">
			...
			{#if on}
				...
				<button type="button" class="inline-flex items-center gap-1 text-primary hover:text-primary-hover" onclick={onView} use:tip={'Turn the view to look straight at the engraving'}>
					<Crosshair size={13} /> Show me
				</button>
				<button type="button" class="inline-flex items-center gap-1 text-text-muted hover:text-text" onclick={onReset} use:tip={'Back to the whole part'}>
					<RotateCcw size={12} /> Whole part
				</button>
				{#if stamps.length > 1}
					<span class="h-4 w-px bg-border"></span>
					<span class="inline-flex items-center gap-1">
						<button type="button" class="inline-flex h-5 w-5 items-center justify-center border border-border text-text hover:border-primary" onclick={() => cycle(-1)} aria-label="Previous face" use:tip={'Previous face (←)'}><ChevronLeft size={13} /></button>
						...
						<button type="button" class="inline-flex h-5 w-5 items-center justify-center border border-border text-text hover:border-primary" onclick={() => cycle(1)} aria-label="Next face" use:tip={'Next face (→)'}><ChevronRight size={13} /></button>
					</span>
					<Popover width="16rem" placement="bottom-end">
