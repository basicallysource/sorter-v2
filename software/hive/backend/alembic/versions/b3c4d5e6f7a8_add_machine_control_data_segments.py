"""add machine control data segments

Revision ID: b3c4d5e6f7a8
Revises: d1f3b5c7e9a2
Create Date: 2026-07-21 12:00:00.000000

Machine -> Hive sync of feeder-dynamics ("control data") capture segments. Each
...
data_type "control_data_segments"). Segments evicted from the machine's local
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("machine_id", "local_id", name="uq_machine_control_data_segments_machine_local"),
    )
    op.create_index(
        "ix_machine_control_data_segments_machine_local_id",
        "machine_control_data_segments",
        ["machine_id", "local_id"],
    )
    op.create_index(
        "ix_machine_control_data_segments_machine_started",
        "machine_control_data_segments",
...
def downgrade() -> None:
    op.drop_index("ix_machine_control_data_segments_machine_started", table_name="machine_control_data_segments")
    op.drop_index("ix_machine_control_data_segments_machine_local_id", table_name="machine_control_data_segments")
    op.drop_table("machine_control_data_segments")
