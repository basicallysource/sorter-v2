...
def mask_center_of_mass(mask: np.ndarray) -> Optional[Tuple[float, float]]:
...
def masks_within_distance(
    ...
    return masks_overlap(mask1, dilated.astype(bool))


def mask_min_distance(object_mask: np.ndarray, target_mask: np.ndarray) -> int:
