...
| P0 | First-run config for the 5-wall C4 platter (install 2026-04-27) | New rotor with 5 physical walls forces pieces into fixed 72° sectors. The carousel handler + sector mode + wall-angle calibration path are wired; the operator-side workflow on first install is the only remaining question. | Step-by-step: (1) install rotor, home machine, drain via the C234 purge, then verify all three trays are physically empty with ``scripts/grab_camera_frame.py 0/1/2 /tmp/cN.jpg``. (2) Capture 60–120 wall frames at varied platter rotations: ``uv run --with websockets python scripts/wall_detector_capture.py --output-dir captures/c4_walls_<date> --duration-s 90 --frame-period-s 0.5 --c4-rpm 1.0`` — script auto-inhibits C1 feed and slowly rotates C4 via ``sample_transport`` while pulling frames off the camera-preview WS. (3) Label the frames with Gemini: ``OPENROUTER_API_KEY=… uv run python scripts/wall_detector_collect.py --input-dir captures/c4_walls_<date> --output-dir datasets/c4_walls_<date> --preview --min-walls 4`` (script defaults already set, ``--preview`` writes overlay JPEGs for spot-checking). (4) Review ``datasets/c4_walls_<date>/previews/`` for any wrong boxes, fix the corresponding ``.txt`` by hand if needed. (5) Train a small YOLO model (single class ``wall``, class_id 0) on the resulting tree. (6) At runtime, post detected wall angles via tuning API: ``{"orchestrator": {"carousel_c4_handler": {"geometry": {"wall_angles_deg": […]}}}}`` — the handler auto-snaps ``classify_deg`` / ``drop_deg`` to the matching sector centers. (7) Visual cross-check from the camera frame; if the snapped angles don't physically match the chute geometry, override them via ``geometry.classify_deg`` / ``geometry.drop_deg``. |
