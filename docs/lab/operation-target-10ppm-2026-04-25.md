...
| P0 | Calibrate ``SectionFeederHandler`` intake/exit geometry per channel | The handler's first live run (2026-04-26 18:14) confirmed the path is wired correctly but the seed ``intake_center_deg = 180°`` is wrong for C2 (matches C3 by ``_upstream_lease_arc_center_rad`` but not C2). C2 pieces clustered between -88° and -47° during section-mode feeding — ≥33° away from the assumed 180° intake — so the handler thought C2 intake was always clear and let C1 over-feed. | Add ``orchestrator.section_feeder_handler.{c2_geometry,c3_geometry,cooldowns_s}`` to the tuning API. Run a short C1-only feed burst, histogram C2 track angles, take the mode (or median) as ``c2_geometry.intake_center_deg``. Same for C3 if its 180° is also off. Then re-run section-mode for a fair PPM comparison against lease mode. |
