...
colored square, and the color says which site you are looking at: Hive is red,
...
`software/hive/frontend/AGENTS.md` § Favicons. Assets are `static/favicon.ico`,
...
single-side colored stripe. The only exception is the servo-state stripe on
`setup/servo/ServoInventoryCard.svelte` (colors set in
`SetupServoOnboardingSection.svelte`), where a left stripe encodes hardware
servo state.
