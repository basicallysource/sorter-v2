...
class CameraFeedPinningTests(unittest.TestCase):
    ...
    def test_describe_overlays_returns_structured_metadata_and_honors_filters(self) -> None:
        ...
        device = _Device(latest=latest, lookup={})
        feed = CameraFeed("carousel", device)
        feed.add_overlay(_MetadataOverlay({"type": "track_bbox", "category": "detections"}))
        feed.add_overlay(_MetadataOverlay({"type": "channel_regions", "category": "regions"}))

        self.assertEqual(
            [
                {"type": "track_bbox", "category": "detections"},
                {"type": "channel_regions", "category": "regions"},
            ],
            feed.describe_overlays(),
        )
        self.assertEqual(
            [{"type": "track_bbox", "category": "detections"}],
            feed.describe_overlays(frozenset({"regions"})),
        )
