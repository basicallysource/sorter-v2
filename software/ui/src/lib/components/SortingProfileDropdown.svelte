<script lang="ts">
	...
	function syncStateToRecentEntry(
		...
		return {
			...
			target_name: syncState.target_name ?? 'SortHive',
		...
		for (const target of profile_library?.targets ?? []) {
			...
			for (const profile of target.profiles ?? []) {
				...
				if (!existing) {
					byProfile.set(key, {
						target_id: target.id,
						target_name: target.name || target.url || 'SortHive',
	...
	async function applyRecentProfile(entry: QuickSwitchProfileEntry) {
		...
		try {
			...
			action_message = payload?.activation_error
				? `Applied ${entry.profile_name} locally. SortHive activation could not be confirmed.`
