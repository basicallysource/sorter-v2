...
| P0 | Document and operationalise the platter-friction bottleneck | The 2026-04-26 acceleration sweep (4 runs, A-D) produced 0.0 / 0.0 / 1.89 / 1.25 PPM. The cleaner finding is that PPM variance across runs with the same software profile is dominated by the physical state of pieces on C3/C2 (sliding vs. carrying). Software hysteresis + headroom-gating do not lift the ceiling. | Out-of-software next steps: visual inspection of the C3/C2 surfaces during a stuck-piece episode; passive C1 weir to cap worst-case dump size; anti-hook geometry. Software side: keep collecting empirical q95 + add the C2 clump-escape (above). |
