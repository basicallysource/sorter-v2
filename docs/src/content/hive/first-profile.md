...
A profile never names a bin number. It only says which boxes exist. Which bin each box lands in is decided on the machine's **Bins** page, once the profile is on it. That page also shows you which bins are the big ones.
