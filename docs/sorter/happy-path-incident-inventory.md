...
| --- | --- | --- |
| `distribution_no_bin_available` | Piece passes through to the bottom tray when no bin is available. | Could be a valid discard path or a bin/profile-capacity incident. |
| `classification_intake_request_timeout` | C4 reopens intake after a C3 request times out. | It is currently a retry guard, but repeated timeouts imply a handoff problem. |
