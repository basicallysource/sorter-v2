...
try:
    ...
    raise SystemExit(
        "onnx is required for this script. Run `uv sync` in software/client before using it."
    ) from exc


CLIENT_ROOT = Path(__file__).resolve().parents[1]
BLOB_DIR = CLIENT_ROOT / "blob"
ZONE_DATASETS_DIR = BLOB_DIR / "zone_datasets"
DEVICE_BENCHMARKS_DIR = BLOB_DIR / "device_benchmarks"
OUTPUT_ROOT = BLOB_DIR / "hailo_compile_bundles"
...
PRESETS: dict[str, HailoPreset] = {
    "classification_chamber_yolo11s": HailoPreset(
        ...
        model_family="yolo",
        hw_arch="hailo8",
        onnx_path=BLOB_DIR / "local_detection_models" / "20260331-zone-classification_chamber-yolo11s" / "exports" / "best.onnx",
    ...
    "classification_chamber_nanodet": HailoPreset(
        ...
        hw_arch="hailo8",
        onnx_path=BLOB_DIR / "local_detection_models" / "20260331-zone-classification_chamber-nanodet" / "exports" / "best.onnx",
