...
<figure class="harness-figure">
  <img src="https://assets.basically.website/sorter-docs/psu-box-inlet-mounting-full-91712ecf9a8b.png" alt="The PSU connections plate drawn twice side by side at the same scale. On the left it is drilled: a 28 by 47.5 mm rectangular cutout for the inlet, two 2.6 mm screw holes 40 mm apart on the cutout's centreline either side of it, and six 12 mm holes below in two columns of three for the DC output jacks. On the right the IEC C14 inlet switch module sits in the cutout, its 48 by 58 mm flange covering the hole with its rocker, fuse drawer and C14 socket showing and its own two screw holes landing on the plate's. A warning says to fit and wire it with no mains cord in the inlet.">
  <figcaption>The connections plate from outside, drilled and then with the inlet in it. <cite>Plate drawn from its STL and the module from its published flange size, both at the same scale, not from a build.</cite></figcaption>
</figure>
