...
Working rules for this directory. Paths below are relative to it unless they
start with `.github/`, which is the sorter-v2 repository root. (It was its own
repository, `basicallysource/parts-calculator`, until 2026-08-20; that repo is
archived.)
...
**Python:** `python3 catalog/generate.py`, with `catalog/requirements.txt`
...
recesses the uid into a face; its module docstring is the spec (sizes, how
faces are found and ranked, the fallbacks), and there is deliberately no
per-part authoring of where the stamp goes. `generate.py` pre-cuts up to four
face variants per printed part and candidate into `build/stamped/`, memoized
on (STL bytes, uid, `engrave.SIGNATURE`); they ride the generated data as
`stamped: [{face, stl, normal, center}]`, best face first, empty when nothing
fits. `all_parts_zip` ships each part's default variant, `all_parts_plain_zip`
the masters. The cut is byte-stable only on one OS (Linux and macOS
triangulate the same solid differently), so a checkout with no memo reuses the
committed stamps of every part whose master and uid are unchanged and cuts
only what changed; a `SIGNATURE` bump re-cuts the lot. One `IdStamp`
component is the whole UI for it. `/u/<uid>` is one prerendered page per uid
the catalog has ever minted.
