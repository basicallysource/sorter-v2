...
| P0 | Run the lease-vs-section A/B from a *physically* drained line | Section feeder is wired, calibrated (`c2_intake_center=-60°`), and back-pressured with `piece_caps` (default 8). Best clean-state section-mode result so far: 1.85 PPM. Lease-mode best: 3.19 PPM. The session's later runs were dominated by accumulated stuck pieces on C3/C4 — both modes delivered 0 distributed when the line started loaded, so the difference between the two modes is currently masked by physical state. | Operator-side: physically clear C3/C4 of any leftover pieces. Then run two back-to-back 180 s windows (lease, then section) with the same controlled-density profile and the simulated chute, and compare distributed counts side-by-side. Section is expected to either match or beat lease; if it matches, the next gap toward Main's 6-7 PPM is in the C4 classification + distributor cycle, not the feeder. |
